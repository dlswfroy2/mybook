...
import { Status as FeeCollectionStatus, FeeCollection, feeCollectionFromDoc } from '@/lib/fees-data';
...
function CollectionReportTab({ allStudents, onDeleteSuccess }: { allStudents: Student[], onDeleteSuccess: () => void }) {
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        const q = query(collection(db, 'feeCollections'), where('academicYear', '==', selectedYear));
        const unsubscribe = onSnapshot(q, (snapshot) => { const data = snapshot.docs.map(doc => feeCollectionFromDoc(doc)).filter((c): c is FeeCollection => c !== null).sort((a, b) => b.collectionDate.getTime() - a.collectionDate.getTime()); setNotices(data); setIsLoading(false); }, (error) => { 
            if (error.code === 'permission-denied') {
                errorEmitter.emit('permission-error', new FirestorePermissionError({ path: 'feeCollections', operation: 'list' })); 
            }
            setIsLoading(false); 
...
export default function AccountsPage() {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-[#F6F7F9] font-kalpurush">
      ...
      <main className={cn("flex-1 p-4 md:p-6 lg:p-10 gap-8 pb-[250px]", activePrintReport && "no-print")}>
                    ...
                    <div key={cls} className="mb-10 break-after-page p-10">
                        <header className="flex items-center gap-6 border-b-4 border-emerald-800 pb-4 mb-6">
                            ...
                            <div className="text-center flex-grow">
                                ...
                                <p className="text-sm font-bold text-slate-700 leading-tight">{schoolInfo.address}</p>
                            ...
                            <tbody>
                                {classStudents.map(student => (
                                    <tr key={student.id} className="h-10">
                                        ...
                                        <td className="text-center">{toBengaliNumber(student.sessionFee || 0)}</td>
                                        <td className="text-center">{toBengaliNumber(student.admissionFee || 0)}</td>
                                        <td className="text-center">{toBengaliNumber(student.otherFee || 0)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
