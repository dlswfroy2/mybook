...
const MarksheetTemplate = ({ result, schoolInfo, examName, academicYear, watermarkOpacity }: any) => {
    ...
    return (
        <div className="marksheet-inner-content border-[1.5px] border-black p-4 h-full flex flex-col bg-transparent relative box-border">
            {schoolInfo.logoUrl && (
                <div className="absolute inset-0 flex items-center justify-center z-0 pointer-events-none watermark-layer opacity-10" style={{ opacity: watermarkOpacity || 0.15 }}>
            ...
            <Card className="bg-white/95 backdrop-blur-md border border-rose-200 shadow-xl rounded-2xl md:rounded-3xl group hover:-translate-y-1 transition-all col-span-2 md:col-span-1">
               <CardContent className="p-4 md:p-5 flex flex-col gap-2">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-sm font-black text-rose-600 leading-tight">এস এস সি পরীক্ষা-{toBengaliNumber(selectedYear)}</p>
                  </div>
                  {stats.sscTotal > 0 ? (
                    <>
